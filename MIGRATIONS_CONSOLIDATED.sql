...
-- ==== 20260315000001_unify_org_id_functions.sql ====
-- Unificar current_org_id() e auth_org_id() em uma única função
...
-- Recriar policies com current_org_id()
DROP POLICY IF EXISTS "ad_accounts_select_org" ON ad_accounts;
CREATE POLICY "ad_accounts_select_org" ON ad_accounts FOR SELECT
  USING (org_id = public.current_org_id());

DROP POLICY IF EXISTS "ad_accounts_insert_org" ON ad_accounts;
CREATE POLICY "ad_accounts_insert_org" ON ad_accounts FOR INSERT
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS "ad_accounts_update_org" ON ad_accounts;
CREATE POLICY "ad_accounts_update_org" ON ad_accounts FOR UPDATE
  USING (org_id = public.current_org_id())
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS "ad_accounts_delete_org" ON ad_accounts;
CREATE POLICY "ad_accounts_delete_org" ON ad_accounts FOR DELETE
  USING (org_id = public.current_org_id());
...
DROP POLICY IF EXISTS "daily_metrics_select_org" ON daily_metrics;
CREATE POLICY "daily_metrics_select_org" ON daily_metrics FOR SELECT
  USING (org_id = public.current_org_id());

DROP POLICY IF EXISTS "daily_metrics_insert_org" ON daily_metrics;
CREATE POLICY "daily_metrics_insert_org" ON daily_metrics FOR INSERT
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS "daily_metrics_update_org" ON daily_metrics;
CREATE POLICY "daily_metrics_update_org" ON daily_metrics FOR UPDATE
  USING (org_id = public.current_org_id())
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS tracking_events_select_org ON tracking_events;
CREATE POLICY tracking_events_select_org ON tracking_events FOR SELECT
  USING (org_id = public.current_org_id());

DROP POLICY IF EXISTS tracking_events_insert_org ON tracking_events;
CREATE POLICY tracking_events_insert_org ON tracking_events FOR INSERT
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS notification_rules_select_org ON notification_rules;
CREATE POLICY notification_rules_select_org ON notification_rules FOR SELECT
  USING (org_id = public.current_org_id());

DROP POLICY IF EXISTS notification_rules_insert_org ON notification_rules;
CREATE POLICY notification_rules_insert_org ON notification_rules FOR INSERT
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS notification_rules_update_org ON notification_rules;
CREATE POLICY notification_rules_update_org ON notification_rules FOR UPDATE
  USING (org_id = public.current_org_id())
  WITH CHECK (org_id = public.current_org_id());

DROP POLICY IF EXISTS notification_rules_delete_org ON notification_rules;
CREATE POLICY notification_rules_delete_org ON notification_rules FOR DELETE
  USING (org_id = public.current_org_id());
